...
This script creates the `Customers`, `Products`, and `Orders` tables based on the schema from the exam paper.
...
### 🚀 Paper 5: SQL Queries and Solutions

Here are the 10 queries from the paper, with their solutions and the expected output based on the sample data above.
...
_Note: This question is phrased ambiguously. A common interpretation for this type of question is to "List products whose price is greater than the average price of the 'Computer' category."_
...
**Answer Query (using CTE):**

```sql
-- Use a Common Table Expression (CTE) to get pending quantities
...
SELECT
    ...
    PO.PendingQuantity
FROM Products P
JOIN PendingOrders PO ON P.ProductID = PO.ProductID
WHERE P.StockQuantity < PO.PendingQuantity;
```
...
**Expected Output:**
A list of customers, ranked by their total spending _only_ on 'Mobile' products.
...
SELECT TOP 10
    C.Name AS CustomerName,
    SUM(O.TotalOrderAmount) AS TotalMobileAmount
FROM Orders O
JOIN Customers C ON O.CustomerID = C.CustomerID
JOIN Products P ON O.ProductID = P.ProductID
WHERE P.Category = 'Mobile'
GROUP BY C.Name
ORDER BY TotalMobileAmount DESC;
```
...
_Note: This output assumes Query 7 (the `DELETE`) has **not** been run. If it was, the 2023-12-20 row would be gone._
